...
import { createInterviewQuestion,createInterviewQuestionResponse, question, AnswerScoreRequest, AnswerScoreResponse } from './interview-types';
...
async function textToAudio(text : string) : Promise<string | null> {
    const data = {
        text: text,
...
async function generateQuestions(
    data : createInterviewQuestion
) : Promise<createInterviewQuestionResponse | null> {
    const {
        role,
        experience,
        customRequirements,
...
function scoringInstructions(round: string, codingLanguage?: string) : string {
    if (round === 'technical') {    
...
async function AnswerScore(
    data : AnswerScoreRequest
) : Promise<AnswerScoreResponse | null> {
    const { question, answer, round, codingLanguage } = data;
...
Answer: ${answer}`;
    ...
    } else if (result && result.response && result.response.candidates && result.response.candidates[0] && result.response.candidates[0].content && result.response.candidates[0].content.parts && result.response.candidates[0].content.parts[0] && result.response.candidates[0].content.parts[0].text) {
