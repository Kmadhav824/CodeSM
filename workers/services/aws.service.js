...
async function getMockData(s3Key) {
  try {
    ...
    const [prob] = await db
      .select({ slug: schema.problem.slug })
      .from(schema.problem)
...
async function fetchTestcasesFromS3(s3Key) {
  try {
    ...
    if (mockData) {
      await redisClient.set(cacheKey, JSON.stringify(mockData), "EX", 86400);
      return mockData;
    }
    ...
    console.error("Error fetching testcases, attempting local mock backup:", err);
    try {
      const mockData = await getMockData(s3Key);
